...
class Parser:
    ...
    def get_courses(self):
        ...
        # names = links.find_all('span', {'class': 'media-body'})
        # for el in names:
        #     print(f"Name of subject - {el.text}")
        for el in links:
            if el.get('data-parent-key') == 'mycourses':
                print(f"Name of subject - {el.select_one('span.media-body').text}")
    ...
    def get_grades(self, id_subject):
        ...
        for i in range(2, len(text)):
            itemname = BeautifulSoup(text[i]['itemname']['content'], 'lxml').text
            grade = text[i]['grade']['content']
